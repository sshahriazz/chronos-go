...
1. [docs/DECISIONS.md](docs/DECISIONS.md) — 35 ADRs. Settled; do not relitigate.
3. [docs/CONVENTIONS.md](docs/CONVENTIONS.md) — **layout, import contract, event
   schema, IDs, errors, idempotency, API and test conventions.** Read before
   writing any Go.
4. [docs/FEATURES.md](docs/FEATURES.md) — feature inventory, all domains.
5. Deep specs (supersede the inventory where they exist):
...
6. [docs/NOTIFICATIONS.md](docs/NOTIFICATIONS.md) — what gets sent, and when.
7. [docs/INFRA.md](docs/INFRA.md) — the running runtime substrate.
8. [docs/EVENT-SOURCING.md](docs/EVENT-SOURCING.md) — **KurrentDB semantics and
   data flows.** Verified against the running server; read before writing any
   command handler, projector or reactor.
9. [docs/REVIEW.md](docs/REVIEW.md) — design review, all findings resolved.
...
*inside* a workspace (Drive, docs, files) are explicitly out of scope — Drive is
...
| Category streams | **unavailable** — `$all` + filter | `$ce-`, `$et-` (need projections) |
...
- **OpenFGA fails closed.** If it is unreachable, deny. This is the one
  deliberate exception to "the server stays resilient" (ADR-010).
...
- **A seat is per person per organization**, not per membership — five
  workspaces, one seat (workspace.md §2).
...
- **Never evaluate permissions in PostgreSQL or Go.** No `permissions` table, no
  ...
  stores what a resource *is*; OpenFGA stores who may touch it.
...
- **Everything in Valkey gets a TTL.** `FLUSHALL` must be survivable.
