...
func (q *Queries) ConfirmEmailReservation(ctx context.Context, arg ConfirmEmailReservationParams) error {
	_, err := q.db.Exec(ctx, ConfirmEmailReservation, arg.EmailIndex, arg.SubjectID)
	return err
}

const DeleteReleasedReservations = `-- name: DeleteReleasedReservations :execrows
...
// live claim.
func (q *Queries) DeleteReleasedReservations(ctx context.Context, releasedAt pgtype.Timestamptz) (int64, error) {
	result, err := q.db.Exec(ctx, DeleteReleasedReservations, releasedAt)
	if err != nil {
		return 0, err
	}
...
func (q *Queries) GetEmailReservation(ctx context.Context, emailIndex string) (EmailReservationView, error) {
	row := q.db.QueryRow(ctx, GetEmailReservation, emailIndex)
	var i EmailReservationView
	err := row.Scan(
		&i.EmailIndex,
		&i.SubjectID,
		&i.Verified,
...
type ListLapsedReservationsRow struct {
	EmailIndex string
	SubjectID  string
	ExpiresAt  pgtype.Timestamptz
}
...
func (q *Queries) ListLapsedReservations(ctx context.Context, arg ListLapsedReservationsParams) ([]ListLapsedReservationsRow, error) {
	rows, err := q.db.Query(ctx, ListLapsedReservations, arg.ExpiresAt, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListLapsedReservationsRow{}
	for rows.Next() {
		var i ListLapsedReservationsRow
		if err := rows.Scan(&i.EmailIndex, &i.SubjectID, &i.ExpiresAt); err != nil {
			return nil, err
		}
...
const ReleaseEmailReservation = `-- name: ReleaseEmailReservation :exec
UPDATE email_reservation_view
SET released_at = $3
WHERE email_index = $1 AND subject_id = $2 AND released_at IS NULL
`

type ReleaseEmailReservationParams struct {
	EmailIndex string
	SubjectID  string
...
func (q *Queries) ReleaseEmailReservation(ctx context.Context, arg ReleaseEmailReservationParams) error {
	_, err := q.db.Exec(ctx, ReleaseEmailReservation, arg.EmailIndex, arg.SubjectID, arg.ReleasedAt)
	return err
}

const TruncateEmailReservations = `-- name: TruncateEmailReservations :exec
...
func (q *Queries) TruncateEmailReservations(ctx context.Context) error {
	_, err := q.db.Exec(ctx, TruncateEmailReservations)
	return err
}

const UpsertEmailReservation = `-- name: UpsertEmailReservation :exec
...
type UpsertEmailReservationParams struct {
	EmailIndex string
	SubjectID  string
	ExpiresAt  pgtype.Timestamptz
...
func (q *Queries) UpsertEmailReservation(ctx context.Context, arg UpsertEmailReservationParams) error {
	_, err := q.db.Exec(ctx, UpsertEmailReservation,
		arg.EmailIndex,
		arg.SubjectID,
		arg.ExpiresAt,
		arg.ReservedAt,
