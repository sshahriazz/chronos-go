...
const DeleteSessionTokens = `-- name: DeleteSessionTokens :execrows
DELETE FROM session_token WHERE session_id = ANY($1::text[])
`
...
// it.
func (q *Queries) DeleteSessionTokens(ctx context.Context, sessionIds []string) (int64, error) {
	result, err := q.db.Exec(ctx, DeleteSessionTokens, sessionIds)
	if err != nil {
		return 0, err
	}
