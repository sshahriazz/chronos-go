...
// something is wrong.
//
// Starting without them would be no better. This binary PROVISIONS: without the
// Stripe key every organization created stays in `provisioning` forever, which
// means no workspace, no invitations and a signup that never completes — and
// nothing raises an error, because from the reactor's side it was simply never
// registered.
//
